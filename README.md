...
이 저장소는 콘텐츠와 코드에 서로 다른 라이선스를 적용하며, 자세한 범위는 [`NOTICE`](./NOTICE)에 정리했습니다.

| 대상 | 라이선스 | 파일 |
|---|---|---|
| 사이트 콘텐츠(요약 텍스트, OG 이미지) | CC BY 4.0 | [`LICENSE`](./LICENSE) |
| 사이트 소스 코드(VitePress 설정, 스크립트, 워크플로) | MIT | [`LICENSE-CODE`](./LICENSE-CODE) |
...
- 원문을 요약·번역·재구성한 문서이며, DORA·Google과 공식 제휴 관계가 없는 **비공식** 문서입니다. 정확한 내용은 반드시 원문을 확인하세요.
- **상표**: "DORA", "Google", "Google Cloud" 및 관련 명칭·로고는 해당 소유자의 상표이며, CC BY 4.0은 상표권을 부여하지 않습니다. 이 프로젝트는 DORA의 로고·브랜드 자산을 사용하지 않습니다.
